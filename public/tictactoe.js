...
function canvasApp(playComputer) {
	//alert("COMPUTER? " + playComputer);
	//alert("HERE");
	var mouseX, mouseY;
	...
	function pickrandomsect() {
		...
		if (openspots.length == 0) return -1;
		var therandspot = Math.floor(Math.random() * openspots.length);
		return openspots[therandspot];
	}
			...
			if (drawLetter(stringtodraw,thesect)) { // if a letter was played (not played if clicked
					...
					if (checkwin()) game_over = true;
						...
						setTimeout( function(){
							...
							if (drawLetter(stringtodraw,pickrandomsect())) {
								x_move = ! x_move;
								if (checkwin()) game_over = true;
							}
						},500);
